      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-8 max-w-5xl mx-auto">
        {testimonials.map((testimonial, index) => (
          <motion.div
            ...
            className="bg-slate-50 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 relative"
          >
            <Quote className="absolute top-6 right-6 sm:top-8 sm:right-8 h-8 w-8 sm:h-12 sm:w-12 text-green-100 rotate-180" />
            ...
            <p className="text-slate-700 text-sm sm:text-lg leading-relaxed mb-6 sm:mb-8 relative z-10">
              &quot;{testimonial.content}&quot;
            </p>

            <div className="flex items-center gap-3 sm:gap-4">
              <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-bold text-lg sm:text-xl">
                {testimonial.author.charAt(0)}
              </div>
              <div>
                <h4 className="font-semibold text-slate-900 text-sm sm:text-base">{testimonial.author}</h4>
